const { Order, OrderItem, Dish, sequelize, User, Table, Category } = require("../models");
  ...
  try {
    ...
    if (reportType === 'all' || reportType === 'products') {
      try {
        ...
        const productsRaw = await sequelize.query(`
          ...
          WHERE o.status = 'paid'
          ${startDate && endDate ? `AND o.created_at BETWEEN '${new Date(startDate).toISOString()}' AND '${new Date(endDate).toISOString()}'` : ''}
    ...
    if (reportType === 'all' || reportType === 'waiters') {
      const waitersData = await Order.findAll({
        ...
        attributes: [
          ...
          [sequelize.fn('AVG', sequelize.col('total_amount')), 'ticketPromedio']
        ],
    ...
    if (reportType === 'all' || reportType === 'financial') {
      ...
      data.financial = financialData.map(item => ({
        ...
        total: parseFloat(item.totalVentas || 0) + parseFloat(item.totalPropinas || 0)
      }));
    }
